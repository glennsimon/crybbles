...
style.type = 'text/css';
const setBgGradient = (upperBgColor) => {
  style.innerHTML = `
...
${LightenDarkenColor(upperBgColor, 100)},
${upperBgColor});
}
#floor-datum {
width: ${floorWidth}px;
...
${LightenDarkenColor(upperBgColor, 50)}B0,
...
const initiateBubble = (bNum, bubbleType, bubbleSize, spinAxis, leftPosDrift, duration, spinDuration) => {
  addBubbleStyle(bNum, bubbleType, bubbleSize);
  ...
  growDatum.classList.add('b' + bubNum + '-grow-datum');
  const newElement = document.createElement('div');
  newElement.classList.add('b' + bubNum + '-element');
  ...
  switch (bubbleType) {
    ...
    case 'dodec':
      for (let i = 1; i < 13; i++) {
        ...
        newFace.classList.add(
          'b' + bubNum + '-dod-face',
          'b' + bubNum + '-dod-face' + i.toString(16).toUpperCase()
        );
        newElement.append(newFace);
      }
      break;
  }
  animateBubble(bNum, spinAxis, leftPosDrift, duration, spinDuration);

  bubNum++;
  if (bubNum > 1.5 * attributes.numberOfBubbles) {
    for (bEng of bubbleEngine) {
      clearInterval(bEng);
    }
    setTimeout(pauseBubbles, 250);
  }
};
...
const addBubbleStyle = (bNum, bubbleType, size) => {
...
const transAnimations = [];
const growAnimations = [];
const spinAnimations = [];

const animateBubble = (bNum, spinAxis, leftPosDrift, duration, spinDuration) => {
  transAnimations.push(
    document.querySelector(`.b${bubNum + td}`).animate(
      [ 
        { 
      ...
      { fill: 'forwards', easing: 'ease-in', duration: duration * 1000 }
    )
  );
  growAnimations.push(
    document.querySelector(`.b${bubNum + gd}`).animate(
      [ 
        { transform: 'scale3d(0.1, 0.1, 0.1)', offset: 0 },
        { transform: 'scale3d(0.5, 0.5, 0.5)', offset: 0.033 },
        { transform: 'scale3d(1, 1, 1)', offset: 0.333 },
      ],
      { fill: 'forwards', duration: duration * 1000 }
    )
  );
  spinAnimations.push(
    document.querySelector(`.b${bubNum + el}`).animate(
      [ 
        { transform: `rotate3d(${spinAxis[0]}, ${spinAxis[1]}, ${spinAxis[2]}, 0deg)` },
        { transform: `rotate3d(${spinAxis[0]}, ${spinAxis[1]}, ${spinAxis[2]}, 360deg)` }
      ],
      { duration: spinDuration * 1000, iterations: Infinity }
    )
  );
}

const animateFloor = () => {
  if (attributes.floorRotPeriod !== 0) {
    document.querySelector('#floor').animate(
      [ { transform: 'rotateZ(360deg)' } ],
      { duration: attributes.floorRotPeriod * 1000, iterations: Infinity}
    );
  }
}
...
// return a random color in the form #hhhhhh
const getNextColor = () => {
  let nextColor = '#';
...
async function generateBubbles() {
  ...
  for (let bNum = 0; bNum < numberOfBubbles; bNum++) {
    ...
    const bubbleType = nucleusData[bNum].bubbleType;
    initiateBubble(bNum, bubbleType, bubbleSize, spinAxis, leftPosDrift, duration, spinDuration);
    bubbleEngine[bubNum] = setInterval(initiateBubble, interval, 
        bNum, bubbleType, bubbleSize, spinAxis, leftPosDrift, duration, spinDuration);
...
const pauseBubbles = () => {
  for (const element of transAnimations) {
    element.pause();
  }
  for (const element of growAnimations) {
    element.pause();
  }
  if (attributes.spinPause) {
    for (const element of spinAnimations) {
      element.pause();
    }
  }
...
const create = (hash) => {
  processHash(hash);
  defineAttributes();
  setBgGradient(attributes.bgBaseColor);
  nucleation();
  generateBubbles();
};
